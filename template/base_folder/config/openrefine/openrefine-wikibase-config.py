...
mediawiki_api_endpoint = 'http://wikibase/w/api.php'

# SPARQL endpoint
wikibase_sparql_endpoint = 'http://wdqs-frontend/proxy/wdqs/bigdata/namespace/wdq/sparql'

# Name of the Wikibase instance
wikibase_name = os.environ['W4R_MW_WIKI_NAME']

# URL of the main page of the Wikibase instance
wikibase_main_page = 'https://' + os.environ['W4R_SERVER_NAME'] + '/wiki/Main_Page'
...
import re
q_re = re.compile(r'(<?http?://wikibase/(entity|wiki)/)?(Q[0-9]+)>?')
q_re_group_id = 3
p_re = re.compile(r'(<?http?://wikibase/(entity/|wiki/Property:))?(P[0-9]+)>?')
p_re_group_id = 3
...
identifier_space = 'https://' + os.environ['W4R_SERVER_NAME'] + '/entity/'
schema_space = 'https://'+ os.environ['W4R_SERVER_NAME'] + '/prop/direct/'
...
qid_url_pattern = 'https://' + os.environ['W4R_SERVER_NAME'] + '/wiki/Item:{{id}}'
...
service_name = 'Reconcile for' + os.environ['W4R_MW_WIKI_NAME'] + '.'

# URL (without the trailing slash) where this server runs
this_host = os.environ['W4R_OPENREFINE_SERVER_NAME']
